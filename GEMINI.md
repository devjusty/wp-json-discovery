...
*   **Plugin Namespaces**: Known WordPress plugin API namespaces are registered in `frontend/src/config/plugins.js`. When a scan discovers a namespace not in this file, the `server` persists it to `server/data/unsupported-plugins.json`.
*   **Logging**: The server records detailed scan activity, performance metrics, and errors into a JSONL file located at `server/data/activity.log`.
